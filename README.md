...
본 프로젝트에서 추가적으로 시도한 부분은, LLM이 특정 웹사이트 환경에 최적화된 답변을 생성할 수 있는지를 검증하는 것이었습니다.
구체적으로는 모델이 사용자의 요청에 대해 웹사이트에 맞는 형식으로 응답하고, 그 결과에서 정확한 값을 추출·필터링하여 사용자에게 일관된 Output을 제공할 수 있는지를 확인하고자 하였습니다.
이를 위해 단순히 시스템 프롬프트에 명령어를 삽입하는 수준을 넘어, Supervised Fine-Tuning(SFT) 과 유사한 접근을 적용하였습니다. 즉, 모델이 특정 지침에 따라 웹사이트 최적화된 답변을 출력하도록 학습/지시를 강화하는 방식으로 실험을 진행하였으며, 이를 통해 사용자 맞춤형 Output 생성 가능성을 탐색하였습니다.
